...
import UnidadDialog from '@/views/catalogs/unidades/UnidadDialog.vue'
import UnidadDrawer from '@/views/catalogs/unidades/UnidadDrawer.vue'
...
const filtered = computed(() => {
  ...
  if (!query) return store.unidades

  return store.unidades.filter(u =>
    (u.clave ?? '').toLowerCase().includes(query)
    || (u.nombre ?? '').toLowerCase().includes(query)
    || (u.abreviatura ?? '').toLowerCase().includes(query),
  )
})
    ...
    <VCardText class="d-flex flex-wrap align-center justify-space-between gap-3">
      <div>
        <div class="d-flex align-center gap-2">
          ...
          <h4 class="text-h5 mb-0">Unidades</h4>
        </div>
        <div class="text-body-2 text-medium-emphasis">
          Crea, edita o elimina unidades de medida para productos.
        </div>
      </div>
    ...
    >
      ...
      <template #item.actions="{ item }">
        <div class="d-flex justify-end gap-1">
          <VTooltip text="Editar unidad" location="top">
          ...
          <VTooltip text="Eliminar unidad" location="top">
            <template #activator="{ props }">
